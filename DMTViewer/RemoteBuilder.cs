...
namespace DMT
{
    internal class RemoteBuilder
    {
        ...
        private void Next()
        {
            ...
            {
                Form.Invoke(new Action(() => { Form.OnLog($"Build completed in {Math.Round((DateTime.Now - Start).TotalSeconds, 2)} seconds", LogType.Event); }));
        ...
        void process_Exited(object sender, EventArgs e)
        {
            ...
            if (pro != null && pro.ExitCode < 0)
            {
                ...
                Form.Invoke(new Action(() => { Form.TryFindError(); }));
                return;
            }
            Next();
